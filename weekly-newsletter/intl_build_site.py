...
CARD_GROUPS = {
    "Europe":           ["Euro Stoxx 50", "CAC 40", "DAX", "FTSE 100"],
    "Asia-Pacific":     ["Nikkei 225", "Hang Seng", "ASX 200"],
    "Emerging Markets": ["MSCI EM"],
}

CARD_ICONS = {
    "Europe":           "\U0001f1ea\U0001f1fa",
    "Asia-Pacific":     "\U0001f30f",
    "Emerging Markets": "\U0001f30d",
}
...
_CSS = """\
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .snapshot-table thead tr, .fx-table thead tr { background: var(--navy); color: var(--white); }
  .snapshot-table thead th, .fx-table thead th {
    padding: 10px 14px;
    text-align: left;
    font-weight: 500;
    letter-spacing: 1px;
  ...
  .snapshot-table tbody tr, .fx-table tbody tr { border-bottom: 1px solid var(--border); }
  .snapshot-table tbody tr:nth-child(even), .fx-table tbody tr:nth-child(even) { background: var(--off-white); }
  .snapshot-table tbody td, .fx-table tbody td { padding: 10px 14px; }
...
def render_html(ctx):
    """Render the international newsletter as a standalone HTML page (v2 design)."""
    ...
    for group_name, members in CARD_GROUPS.items():
        ...
        for member in members:
            ...
            rows += f'<div class="idx-row"><span>{member}</span><span class="{cls}">{pct:+.2f}%</span></div>\n'
        cards_html += f"""
    ...
    best_str = f"{best.get('name', '')} ({best.get('weekly_pct', 0):+.2f}%)" if best else ""
    worst_str = f"{worst.get('name', '')} ({worst.get('weekly_pct', 0):+.2f}%)" if worst else ""
    ...
    for ev in ctx["past_events"]:
        ...
        if "cpi" in event_lower or "inflation" in event_lower:
            ...
            emoji = "\U0001f3ed"
        elif "ecb" in event_lower or "boj" in event_lower or "boe" in event_lower or "central bank" in event_lower:
            emoji = "\U0001f3e6"
        elif "trade" in event_lower or "export" in event_lower:
            emoji = "\U0001f6a2"
        else:
            emoji = "\U0001f4c8"
        analysis_cards += f"""
...
<head>
  ...
  <title>Framework Foundry \u2014 International Edition \u00b7 {date_str}</title>
...
<div class="page">
  ...
  <header class="header">
    <div class="header-inner">
      ...
      <div class="logo-text">
        ...
        <span class="logo-tagline">International Edition &nbsp;\u00b7&nbsp; Research for the serious investor</span>
      </div>
      <div class="header-meta">
        <span class="issue-label">Week Ending</span>
        <span class="issue-date">{display_date}</span>
        <span class="issue-week">International Edition</span>
      </div>
    </div>
  ...
  <div class="region-banner">
    <span>Coverage: Europe \u00b7 Asia-Pacific \u00b7 Emerging Markets \u00b7 FX \u00b7 Macro</span>
    <div class="region-dots">
      <span class="region-dot">\U0001f1ea\U0001f1fa</span>
      <span class="region-dot">\U0001f1ef\U0001f1f5</span>
      <span class="region-dot">\U0001f30f</span>
    </div>
  </div>
  ...
  <div class="content">
    ...
    <div class="section-block">
      ...
      <div class="fx-title">FX Rates</div>
  ...
  <div class="subscribe-section">
    ...
    <p>Free weekly market intelligence, every Friday</p>
  ...
  <div class="share-bar">
    ...
    <div class="share-buttons">
      <a class="share-btn twitter" href="#" target="_blank" rel="noopener"
         onclick="this.href='https://twitter.com/intent/tweet?text='+encodeURIComponent('Global markets this week via Framework Foundry International \u2014  '+window.location.href);return true;"
  ...
  <!-- FOOTER -->
  <footer class="footer">
    <div class="footer-logo">FRAMEWORK <span>FOUNDRY</span> &nbsp;\u00b7&nbsp; <span style="color:rgba(255,255,255,0.35); font-size:10px; letter-spacing:1px;">INTERNATIONAL EDITION</span></div>
