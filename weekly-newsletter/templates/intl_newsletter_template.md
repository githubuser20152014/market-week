...
|-------|--------|------:|--------:|-----------:|
{% for idx in indices -%}
| {{ idx.name }} | {{ idx.region }} | {{ "{:,.2f}".format(idx.close) }} | {{ "%+.2f"|format(idx.weekly_pct) }}% | {{ "{:,.2f}".format(idx.week_low) }} - {{ "{:,.2f}".format(idx.week_high) }} |
{% endfor %}
...
|------|-----:|--------:|
{% for fx in fx_rates -%}
| {{ fx.name }} | {{ "%.4f"|format(fx.rate) }} | {{ "%+.2f"|format(fx.weekly_pct) }}% |
{% endfor %}
